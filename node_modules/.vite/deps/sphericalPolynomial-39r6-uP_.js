import { t as Clamp } from "./math.scalar.functions-DMYdgtAr.js";
import { n as Observable } from "./observable-Dg5nDoxE.js";
import { a as Vector3, i as Vector2, r as TmpVectors, t as Matrix, y as _ObserveArray } from "./math.vector-ivCpDlc6.js";
import { r as RegisterClass, t as GetClass } from "./typeStore-Dy-yCYbL.js";
import { t as EngineStore } from "./engineStore-Cs1hlddx.js";
import "./math.color-BkO_SHWK.js";
import { t as Logger } from "./logger-CE6knO9r.js";
import { i as serialize, s as serializeAsColor4, v as __decorate } from "./decorators-C0OZDeZS.js";
import { n as SerializationHelper } from "./math.size-DQcnuHgY.js";
import { A as _RetryWithInterval, a as NearestPOT, h as Effect, n as GetExponentOfTwo, o as AbstractEngine, t as FloorPOT } from "./tools.functions-B2mppQhq.js";
import { a as PostProcessManager, o as SmartArray, r as RenderingManager } from "./math.frustum-Bu-6x_FW.js";
import { t as UniformBuffer } from "./uniformBuffer-DLLFhL0q.js";
import "./math.plane-Bx7jd09E.js";
import { t as LightConstants } from "./lightConstants-BL0efsdb.js";
import "./math.path-hY2PRwLR.js";
import { n as EffectWrapper } from "./effectRenderer-DyZWNPGG.js";
...
var ObjectRenderer = class ObjectRenderer {
	...
	*/
	...
	set disableImageProcessing(value) {
		if (value === this._disableImageProcessing) return;
	...
	*/
	...
	set name(value) {
		...
		this._name = value;
		if (this._sceneUBOs) for (let i = 0; i < this._sceneUBOs.length; ++i) this._sceneUBOs[i].name = `Scene ubo #${i} for ${this.name}`;
	...
	*/
	setMaterialForRendering(mesh, material) {
		...
		for (let j = 0; j < meshes.length; ++j) for (let i = 0; i < this.options.numPasses; ++i) {
			let mesh = meshes[j];
			if (meshes[j].isAnInstance) mesh = meshes[j].sourceMesh;
			mesh.setMaterialForRenderPass(this._renderPassIds[i], material !== void 0 ? Array.isArray(material) ? material[i] : material : void 0);
		}
	}
	/** @internal */
	...
	*/
	constructor(name, scene, options) {
		...
		this._renderListHasChanged = (_functionName, previousLength) => {
			...
			if (previousLength === 0 && newLength > 0 || newLength === 0) for (const mesh of this._scene.meshes) mesh._markSubMeshesAsLightDirty();
		};
		/**
		...
		*/
		...
		if (this.options.enableClusteredLights) this.onInitRenderingObservable.add(() => {
			for (const light of this._scene.lights) if (light.getTypeID() === LightConstants.LIGHTTYPEID_CLUSTERED_CONTAINER && light.isSupported) light._updateBatches(this.activeCamera).render();
		});
		this._scene.addObjectRenderer(this);
	}
	_releaseRenderPassId() {
		for (let i = 0; i < this.options.numPasses; ++i) this._engine.releaseRenderPassId(this._renderPassIds[i]);
		this._renderPassIds.length = 0;
	}
	_createRenderPassId() {
		this._releaseRenderPassId();
		for (let i = 0; i < this.options.numPasses; ++i) this._renderPassIds[i] = this._engine.createRenderPassId(`${this.name}#${i}`);
	}
	_createSceneUBO(name, isMultiview) {
		const ubo = new UniformBuffer(this._scene.getEngine(), void 0, isMultiview, name, void 0, false);
		ubo.addUniform("viewProjection", 16);
		if (isMultiview) ubo.addUniform("viewProjectionR", 16);
	...
	*/
	shouldRender() {
		if (this._engine.snapshotRendering) return true;
	...
	*/
	prepareRenderList() {
		...
		if (this._disableImageProcessing) this._scene.imageProcessingConfiguration._applyByPostProcess = this._disableImageProcessing;
	}
	/**
	...
	*/
	initRender(viewportWidth, viewportHeight) {
		...
		if (camera) {
			if (!this.dontSetTransformationMatrix) this._scene.setTransformMatrix(camera.getViewMatrix(), camera.getProjectionMatrix(true));
			this._scene.activeCamera = camera;
			this._engine.setViewport(camera.rigParent ? camera.rigParent.viewport : camera.viewport, viewportWidth, viewportHeight);
		}
		if (this._useUBO) this._scene.finalizeSceneUbo();
	...
	*/
	finishRender() {
		const scene = this._scene;
		if (this._useUBO) this._scene.setSceneUniformBuffer(this._currentSceneUBO);
		if (this._disableImageProcessing) scene.imageProcessingConfiguration._applyByPostProcess = this._currentApplyByPostProcessSetting;
	...
	*/
	render(passIndex = 0, skipOnAfterRenderObservable = false) {
		...
		if (!(this._engine.snapshotRendering && this._engine.snapshotRenderingMode === 1)) {
			...
			if (outlineRenderer) outlineRenderer.enabled = this.enableOutlineRendering;
			...
			if (outlineRenderer) outlineRenderer.enabled = outlineRendererIsEnabled;
	...
	_checkReadiness() {
		...
		for (let passIndex = 0; passIndex < numPasses && returnValue; passIndex++) {
			...
			if (this.getCustomRenderList) {
				...
				if (currentRenderList) currentRenderListLength = currentRenderList.length;
			}
	...
	_prepareRenderingManager(passIndex = 0, winterIsComing = false) {
		...
		if (!currentRenderList) {
			if (this._defaultRenderListPrepared && !winterIsComing && !this._engine.isWebGPU) return defaultRenderList;
		...
		if (this.enableBoundingBoxRendering && boundingBoxRenderer && winterIsComing) for (let i = 0; i < boundingBoxRenderer.renderList.length; i++) {
	...
	*/
	dispose() {
		...
		for (let i = 0; i < renderListLength; i++) {
			...
			if (mesh && mesh.getMaterialForRenderPass(this.renderPassId) !== void 0) mesh.setMaterialForRenderPass(this.renderPassId, void 0);
		}
		...
		if (this._sceneUBOs) for (const ubo of this._sceneUBOs) ubo.dispose();
	...
	constructor(name, size, scene, generateMipMaps = false, doNotChangeAspectRatio = true, type = 0, isCube = false, samplingMode = Texture.TRILINEAR_SAMPLINGMODE, generateDepthBuffer = true, generateStencilBuffer = false, isMulti = false, format = 5, delayAllocation = false, samples, creationFlags, noColorAttachment = false, useSRGBBuffer = false) {
		...
		*/
		...
		this._onBeforeRenderingManagerRenderObserver = this._objectRenderer.onBeforeRenderingManagerRenderObservable.add(() => {
			const scene = this._scene;
			if (!this._disableEngineStages) for (const step of scene._beforeRenderTargetClearStage) step.action(this, this._currentFaceIndex, this._currentLayer);
			if (this.onClearObservable.hasObservers()) this.onClearObservable.notifyObservers(engine);
			else if (!this.skipInitialClear) engine.clear(this.clearColor ?? scene.clearColor, true, true, true);
			if (!this._doNotChangeAspectRatio) scene.updateTransformMatrix(true);
			if (!this._disableEngineStages) for (const step of scene._beforeRenderTargetDrawStage) step.action(this, this._currentFaceIndex, this._currentLayer);
			if (engine._debugPushGroup) engine._debugPushGroup(`Render to ${this.name} (face #${this._currentFaceIndex} layer #${this._currentLayer})`);
		});
		this._onAfterRenderingManagerRenderObserver = this._objectRenderer.onAfterRenderingManagerRenderObservable.add(() => {
			if (engine._debugPopGroup) engine._debugPopGroup();
	...
	*/
	isReadyForRendering() {
		if (!this._dumpToolsLoading) {
			...
			import("./dumpTools-FJ8RS5ql.js").then((n) => n.a).then((module) => this._dumpTools = module);
		}
	...
	_render(useCameraPostProcess = false, dumpForDebug = false) {
		...
		if (engine._debugPushGroup) engine._debugPushGroup(`Render to ${this.name}`);
		this._objectRenderer.prepareRenderList();
		this.onBeforeBindObservable.notifyObservers(this);
		this._objectRenderer.initRender(this.getRenderWidth(), this.getRenderHeight());
	...
	_prepareFrame(scene, faceIndex, layer, useCameraPostProcess) {
		if (this._postProcessManager) {
			if (!this._prePassEnabled) {
				if (!this._postProcessManager._prepareFrame(this._texture, this._postProcesses)) this._bindFrameBuffer(faceIndex, layer);
			}
...
var PostProcess = class PostProcess {
	...
	*/
	...
	_gatherImports(useWebGPU = false, list) {
		if (useWebGPU && this._webGPUReady) list.push(Promise.all([import("./postprocess.vertex-BCJsk_t4.js").then((n) => n.n)]));
		else list.push(Promise.all([import("./postprocess.vertex-B_jlHyIn.js").then((n) => n.n)]));
	}
	/**
...
var ThinPassPostProcess = class ThinPassPostProcess extends EffectWrapper {
	_gatherImports(useWebGPU, list) {
		if (useWebGPU) {
			...
			list.push(Promise.all([import("./pass.fragment-cZ6xB1Fg.js").then((n) => n.n)]));
		} else list.push(Promise.all([import("./pass.fragment-BbGz-xqg.js").then((n) => n.n)]));
	...
	*/
	constructor(name, engine = null, options) {
		...
		if (!localOptions.engine) localOptions.engine = EngineStore.LastCreatedEngine;
		super(localOptions);
	}
};
/**
...
var ThinPassCubePostProcess = class ThinPassCubePostProcess extends EffectWrapper {
	_gatherImports(useWebGPU, list) {
		if (useWebGPU) {
			this._webGPUReady = true;
			list.push(Promise.all([import("./passCube.fragment-BYFQKF3Q.js").then((n) => n.n)]));
		} else list.push(Promise.all([import("./passCube.fragment-C6uZY0Mn.js").then((n) => n.n)]));
		super._gatherImports(useWebGPU, list);
	}
	/**
...
async function WhenTextureReadyAsync(texture) {
	...
	if (texture.loadingError) throw new Error(texture.errorObject?.message || `Texture ${texture.name} errored while loading.`);
	const onLoadObservable = texture.onLoadObservable;
	if (onLoadObservable) return await new Promise((res) => onLoadObservable.addOnce(() => res()));
	const onLoadedObservable = texture._texture?.onLoadedObservable;
	if (onLoadedObservable) return await new Promise((res) => onLoadedObservable.addOnce(() => res()));
...
async function ReadPixelsUsingRTT(texture, width, height, face, lod, slice = 0) {
	...
	if (!engine.isWebGPU) if (is3DTexture) await import("./lod3D.fragment-DRrCgfct.js");
	else if (texture.isCube) await import("./lodCube.fragment-C7pwpy-G.js").then((n) => n.n);
	else await import("./lod.fragment-owGB1qg2.js").then((n) => n.n);
	else if (is3DTexture) await import("./lod3D.fragment-zXASbsul.js");
	else if (texture.isCube) await import("./lodCube.fragment-ZJyHlK4r.js").then((n) => n.n);
	else await import("./lod.fragment-us60dxx6.js").then((n) => n.n);
	let lodPostProcess;
	if (is3DTexture) lodPostProcess = new PostProcess("lod3D", "lod3D", {
		uniforms: [
			"lod",
			"gamma",
			"slice"
	...
	else if (!texture.isCube) lodPostProcess = new PostProcess("lod", "lod", {
		uniforms: ["lod", "gamma"],
		samplingMode: Texture.NEAREST_NEAREST_MIPNEAREST,
		engine,
		shaderLanguage: engine.isWebGPU ? 1 : 0
	...
	lodPostProcess.onApply = function(effect) {
		...
		if (is3DTexture) effect.setFloat("slice", slice);
	};
	try {
...
async function GetTextureDataAsync(texture, width, height, face = 0, lod = 0, forceRTT = false, slice = 0) {
	...
	if (forceRTT || is3DTexture || IsCompressedTextureFormat(texture.textureFormat) || targetWidth !== textureWidth || targetHeight !== textureHeight) {
		if (texture.is2DArray) throw new Error(`Reading pixels from 2D array textures with ${forceRTT ? "RTT" : "compression"} is not supported.`);
	...
	if (!data) throw new Error(`Failed to read pixels from texture ${texture.name}.`);
...
//#endregion
//#region node_modules/@babylonjs/core/Maths/math.vertexFormat.js
/**
* Contains position and normal vectors for a vertex
*/
var PositionNormalVertex = class PositionNormalVertex {
	/**
...
*/
var PositionNormalTextureVertex = class PositionNormalTextureVertex {
	/**
...
//#endregion
export { RenderTargetTexture as _, ApplyPostProcess as a, Coordinate as b, GetTextureDataAsync as c, WhenTextureReadyAsync as d, PassCubePostProcess as f, PostProcess as g, ThinPassPostProcess as h, PositionNormalVertex as i, TextureTools as l, ThinPassCubePostProcess as m, SphericalPolynomial as n, CreateResizedCopy as o, PassPostProcess as p, PositionNormalTextureVertex as r, FromHalfFloat as s, SphericalHarmonics as t, ToHalfFloat as u, ObjectRenderer as v, Space as x, Axis as y };

//# sourceMappingURL=sphericalPolynomial-39r6-uP_.js.map