...
import { n as Tools } from "./tools-CjHzo3M6.js";
import { i as serialize, v as __decorate } from "./decorators-C0OZDeZS.js";
import { n as RandomGUID } from "./guid-WFMbp9X_.js";
import { t as PointerEventTypes } from "./pointerEvents-Ba0YlU2U.js";
import { E as getRichTypeByFlowGraphType, o as defaultValueSerializationFunction } from "./flowGraphBlock-BNLz85Bs.js";
import { n as GetFlowGraphAssetWithType } from "./flowGraphAssetsContext-6Svp1IuH.js";
import { t as FlowGraphExecutionBlock } from "./flowGraphExecutionBlock-t9izP0uO.js";
import { t as FlowGraphAsyncExecutionBlock } from "./flowGraphAsyncExecutionBlock-EE6AL5OA.js";
import { a as _IsDescendantOf } from "./utils-B1LyBAnv.js";
import { t as FlowGraphEventBlock } from "./flowGraphEventBlock-LKPvrcRl.js";
...
var FlowGraphContext = class {
	...
	*/
	_shouldBreak(block, signal) {
		...
		if (this._pendingActivation) return true;
		if (this._stepMode) {
			this._stepMode = false;
	...
	*/
	serialize(serializationObject = {}, valueSerializationFunction = defaultValueSerializationFunction) {
		...
		if (Object.keys(this._variableTypes).length > 0) serializationObject._variableTypes = { ...this._variableTypes };
...
__decorate([serialize()], FlowGraphContext.prototype, "name", void 0);
//#endregion
...
//#endregion
//#region node_modules/@babylonjs/core/FlowGraph/flowGraphValidator.js
/**
...
var NumericLikeTypes = new Set([
	"number",
	"FlowGraphInteger",
	"boolean"
]);
var VectorLikeTypes = new Set(["Vector4", "Quaternion"]);
var ColorLikeTypes = new Set(["Color3", "Color4"]);
/**
* @internal
*/
function _AreTypesCompatible(sourceType, targetType) {
	...
	if (NumericLikeTypes.has(sourceType) && NumericLikeTypes.has(targetType)) return true;
	if (VectorLikeTypes.has(sourceType) && VectorLikeTypes.has(targetType)) return true;
	if (ColorLikeTypes.has(sourceType) && ColorLikeTypes.has(targetType)) return true;
	return false;
}
/**
...
function ValidateFlowGraph(flowGraph) {
	...
	if (_GetEventBlocks(flowGraph).length === 0) addIssue({
		severity: 0,
		message: "Graph has no event blocks — nothing will trigger execution."
	});
	for (const block of allBlocks) for (const input of block.dataInputs) if (!input.optional && !input.isDisabled && !input.isConnected()) addIssue({
		severity: 1,
		message: `"${input.name}" is not connected and will use its default value.`,
		block,
		connectionName: input.name
	});
	for (const block of allBlocks) if (block instanceof FlowGraphExecutionBlock) {
		if (_IsEventBlock(block)) continue;
		const inSignal = block.signalInputs.find((s) => s.name === "in");
		if (inSignal && !inSignal.isConnected()) addIssue({
			severity: 0,
	...
	for (const block of allBlocks) for (const input of block.dataInputs) {
		...
		if (srcType && tgtType && !_AreTypesCompatible(srcType, tgtType)) {
			if (source.richType.typeTransformer !== void 0 || input.richType.typeTransformer !== void 0) continue;
			addIssue({
				severity: 1,
	...
	return {
		isValid: issues.every((i) => i.severity !== 0),
		issues,
...
function ValidateFlowGraphWithBlockList(flowGraph, allKnownBlocks) {
	...
	for (const block of allKnownBlocks) if (!reachableIds.has(block.uniqueId)) {
		const issue = {
			severity: 1,
		...
		arr.push(issue);
		for (const input of block.dataInputs) if (!input.optional && !input.isDisabled && !input.isConnected()) {
			const dataIssue = {
				severity: 1,
				message: `"${input.name}" is not connected and will use its default value.`,
				block,
	...
	result.isValid = result.issues.every((i) => i.severity !== 0);
...
function _GetEventBlocks(flowGraph) {
	const eventBlocks = [];
	for (const type in flowGraph._eventBlocks) for (const block of flowGraph._eventBlocks[type]) eventBlocks.push(block);
	return eventBlocks;
}
/**
...
function _DetectDataCycles(allBlocks, addIssue) {
	...
	for (const block of allBlocks) color.set(block.uniqueId, white);
	const blockMap = /* @__PURE__ */ new Map();
	for (const block of allBlocks) blockMap.set(block.uniqueId, block);
	...
	function dfs(block) {
		...
		for (const input of block.dataInputs) {
			...
			for (const connected of input._connectedPoint) {
				...
				if (dep instanceof FlowGraphExecutionBlock) continue;
	...
	for (const block of allBlocks) if (color.get(block.uniqueId) === white) dfs(block);
}
//#endregion
...
var FlowGraph = class {
	...
	*/
	removeContext(index) {
		if (index < 0 || index >= this._executionContexts.length) return;
	...
	*/
	addBlock(block) {
		if (this._allBlocks.indexOf(block) === -1) this._allBlocks.push(block);
	}
	/**
	...
	*/
	removeBlock(block) {
		...
		if (idx !== -1) this._allBlocks.splice(idx, 1);
		if (block instanceof FlowGraphExecutionBlock && "type" in block) {
			...
			if (list) {
				...
				if (eIdx !== -1) list.splice(eIdx, 1);
			}
		}
		if (block instanceof FlowGraphAsyncExecutionBlock) for (const context of this._executionContexts) {
			block._cancelPendingTasks(context);
			block._resetAfterCanceled(context);
		}
		for (const input of block.dataInputs) input.disconnectFromAll();
		for (const output of block.dataOutputs) output.disconnectFromAll();
		if (block instanceof FlowGraphExecutionBlock) {
			for (const signalIn of block.signalInputs) signalIn.disconnectFromAll();
			for (const signalOut of block.signalOutputs) signalOut.disconnectFromAll();
		}
	}
	/**
	...
	start() {
		...
		if (!resumingFromPause) {
			...
			if (this._scene.isReady(true)) {
				this._sceneEventCoordinator.sceneReadyTriggered = true;
				this._sceneEventCoordinator.onEventTriggeredObservable.notifyObservers({ type: "SceneReady" });
			} else this._scene.executeWhenReady(() => {
		...
		if (type === "MeshPick") {
			...
			for (const block1 of order) {
				...
				for (; i < order.length; i++) {
					...
					if (mesh1 && mesh2 && _IsDescendantOf(mesh1, mesh2)) break;
				}
	...
	*/
	serialize(serializationObject = {}, valueSerializeFunction) {
		...
		const serializeBlock = (block) => {
			if (seen.has(block.uniqueId)) return;
		...
		for (const block of this._allBlocks) serializeBlock(block);
...
var FlowGraphCoordinator = class FlowGraphCoordinator {
	constructor(config) {
		...
		*/
		...
		this._onBeforeRenderObserver = this.config.scene.onBeforeRenderObservable.add(() => {
			...
			if (executeOnNextFrame.length) for (const event of executeOnNextFrame) {
				...
				if (index !== -1) this._executeOnNextFrame.splice(index, 1);
			}
		});
	...
	*/
	start() {
		for (const graph of this._flowGraphs) graph.start();
	}
	/**
	* Disposes all graphs
	*/
	dispose() {
		for (const graph of this._flowGraphs) graph.dispose();
	...
	notifyCustomEvent(id, data, async = !this.dispatchEventsSynchronously) {
		...
		if (this._eventExecutionCounter.has(id)) {
			...
			if (count >= FlowGraphCoordinator.MaxEventTypeExecutionPerFrame) {
				if (count === FlowGraphCoordinator.MaxEventTypeExecutionPerFrame) Logger.Warn(`FlowGraphCoordinator: Too many executions of event "${id}".`);
				return;
			}
...
//#endregion
export { FlowGraphState as a, ValidateFlowGraphWithBlockList as c, FlowGraphLogger as d, FlowGraph as i, FlowGraphContext as l, flowGraphReceiveCustomEventBlock_exports as n, FlowGraphValidationSeverity as o, FlowGraphCoordinator as r, ValidateFlowGraph as s, FlowGraphReceiveCustomEventBlock as t, FlowGraphAction as u };

//# sourceMappingURL=flowGraphReceiveCustomEventBlock-CNTqg0yQ.js.map