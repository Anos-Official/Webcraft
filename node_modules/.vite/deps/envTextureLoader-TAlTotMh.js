...
import { t as Scene } from "./scene-D2FlR6h_.js";
import { a as Vector3 } from "./math.vector-ivCpDlc6.js";
import { t as Logger } from "./logger-CE6knO9r.js";
import { D as GetBlobBufferSource, n as Tools } from "./tools-CjHzo3M6.js";
import { c as InternalTexture } from "./tools.functions-B2mppQhq.js";
import { a as ApplyPostProcess, g as PostProcess, n as SphericalPolynomial } from "./sphericalPolynomial-39r6-uP_.js";
import { o as BaseTexture } from "./texture-DAPDq6-l.js";
import { n as DumpDataAsync } from "./dumpTools-FJ8RS5ql.js";
import { t as CubeMapToSphericalPolynomialTools } from "./cubemapToSphericalPolynomial-DCG7rb7n.js";
...
var RGBDTextureTools = class {
	...
	*/
	static ExpandRGBDTexture(texture) {
		...
		const expandRgbdTextureAsync = async () => {
			...
			internalTexture.isReady = false;
			if (isWebGpu) await import("./rgbdDecode.fragment-DqPvD5FY.js").then((n) => n.n);
			else await import("./rgbdDecode.fragment-DFSxXjxM.js").then((n) => n.n);
		...
		else texture.onLoadObservable.addOnce(expandRgbdTextureAsync);
	}
	/**
	...
	*/
	static async EncodeTextureToRGBD(internalTexture, scene, outputTextureType = 0) {
		if (!scene.getEngine().isWebGPU) await import("./rgbdEncode.fragment-C18m4NL-.js").then((n) => n.n);
		else await import("./rgbdEncode.fragment-Y4yN-2Cd.js").then((n) => n.n);
...
async function CreateEnvTextureAsync(texture, options = {}) {
	...
	if (!internalTexture) return await Promise.reject("The cube texture is invalid.");
	const engine = internalTexture.getEngine();
	if (texture.textureType !== 2 && texture.textureType !== 1 && texture.textureType !== 0 && texture.textureType !== 0 && texture.textureType !== 7 && texture.textureType !== -1) return await Promise.reject("The cube texture should allow HDR (Full Float or Half Float).");
	let textureType = 1;
	if (!engine.getCaps().textureFloatRender) {
		textureType = 2;
		if (!engine.getCaps().textureHalfFloatRender) return await Promise.reject("Env texture can only be created when the browser supports half float or full float rendering.");
	}
	...
	for (let i = 0; i <= mipmapsCount; i++) {
		...
		for (let face = 0; face < 6; face++) specularTextures[i * 6 + face] = await _GetTextureEncodedDataAsync(hostingScene, texture, textureType, face, i, faceWidth, imageType, options.imageQuality);
	}
	...
	if (irradianceTexture) {
		...
		for (let face = 0; face < 6; face++) diffuseTextures[face] = await _GetTextureEncodedDataAsync(hostingScene, irradianceTexture, textureType, face, 0, faceWidth, imageType, options.imageQuality);
	}
...
function UploadEnvLevelsAsync(texture, data, info) {
	...
	if (irradianceTexture) {
		...
		if (info.irradiance?.irradianceTexture?.dominantDirection) dominantDirection = Vector3.FromArray(info.irradiance.irradianceTexture.dominantDirection);
...
async function _UploadLevelsAsync(texture, imageData, canGenerateNonLODTextures, imageType = DefaultEnvironmentTextureImageType) {
	...
	if (expandTexture) {
		if (engine.isWebGPU) {
			shaderLanguage = 1;
			await import("./rgbdDecode.fragment-DqPvD5FY.js").then((n) => n.n);
		} else await import("./rgbdDecode.fragment-DFSxXjxM.js").then((n) => n.n);
	...
	for (let i = 0; i < imageData.length; i++) for (let face = 0; face < 6; face++) {
		...
		let promise;
		if (engine._features.forceBitmapOverHTMLImageElement) promise = engine.createImageBitmap(blob, {
...
var envTextureLoader_exports = /* @__PURE__ */ __exportAll({ _ENVTextureLoader: () => _ENVTextureLoader });
/**
...
//#endregion
export { CreateRadianceImageDataArrayBufferViews as a, UploadEnvLevelsAsync as c, UploadRadianceLevelsAsync as d, _UpdateRGBDAsync as f, CreateIrradianceImageDataArrayBufferViews as i, UploadEnvSpherical as l, RGBDTextureTools as m, envTextureLoader_exports as n, EnvironmentTextureTools as o, normalizeEnvInfo as p, CreateEnvTextureAsync as r, GetEnvInfo as s, _ENVTextureLoader as t, UploadIrradianceLevelsAsync as u };

//# sourceMappingURL=envTextureLoader-TAlTotMh.js.map